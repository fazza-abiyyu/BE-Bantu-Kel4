...
exports.refreshToken = async (req, res) => {
    try {
        ...
        if (!refreshToken) {
            return res.status(400).json({message: 'Tidak ada refresh token yang ditemukan dalam cookie.'});
        }
        ...
        if (!user) {
            return res.status(403).json({message: 'Pengguna tidak valid dengan refresh token.'});
        }
...
exports.logout = async (req, res) => {
    try {
        ...
        return res.status(500).json({statusMessage: 'Internal Server Error', error: error.message});
    }
};